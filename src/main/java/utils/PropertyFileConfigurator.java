...
import annotation.factory.Config;
...
/**
 * Класс-конфигуратор для компонентов, помеченных аннотацией {@link ConfigProperty}.
 */
public class PropertyFileConfigurator implements Config {
    ...
    /**
     * Метод проводит конфигурацию поля, помеченного аннотацией {@link ConfigProperty}.
     ...
     */
    @Override
    public void configure(Object component) throws IllegalAccessException, IOException {
        Class<?> clazz = component.getClass();
        for (Field field : clazz.getDeclaredFields()) {
            if (field.isAnnotationPresent(ConfigProperty.class)) {
                ConfigProperty annotation = field.getAnnotation(ConfigProperty.class);

                String configName = annotation.fileName();
                if (configName.isEmpty()) {
                    configName = DataPath.PROPERTY_FILE.getPath();
                }

                String propertyName = annotation.propertyKey();
    ...
    private <T> T getValueWithType(Class<T> type, String value) {
        if (value == null) {
            throw new TechnicalException("В файле конфигурации отсутствует значение");
        }
